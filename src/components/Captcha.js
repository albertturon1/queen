        ...
        <div className="w-full h-full absolute top-0 right-0  bg-gray-300 z-20 px-5 pt-8">
            ...
            <div className="flex flex-row gap-x-4 mt-3">
                {question?.answers.map((answer, index) => {
                    return (
                        <div key={index} className="flex flex-row justify-center items-center border border-black px-5 py-2 rounded-sm hover:cursor-pointer"
                            onClick={() => onCaptchaSubmit(answer === question.correctAnswer ? true : false)}>
